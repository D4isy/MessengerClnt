...
void CMessaengerClientDlg::OnClickedLogin()
{
	...
	if (editID.GetLength() == 0) {
		...
		MessageBox(_T("아이디를 입력해주세요."), _T("로그인"), MB_ICONERROR);
		return;
	}
	...
	if (editPASS.GetLength() == 0) {
		MessageBox(_T("비밀번호를 입력해주세요."), _T("로그인"), MB_ICONERROR);
		return;
	}
