...
BEGIN_MESSAGE_MAP(CTreeCtrlDrag, CTreeCtrl)
	...
	ON_NOTIFY(TVN_BEGINDRAG, TREE_FRIEND, OnBeginDrag)
	ON_NOTIFY(TVN_BEGINLABELEDIT, TREE_FRIEND, OnBeginLabelEdit)
	ON_NOTIFY(TVN_ENDLABELEDIT, TREE_FRIEND, OnEndLabelEdit)
	ON_WM_TIMER()
	ON_WM_MOUSEMOVE()
	ON_WM_MOUSEWHEEL()
	ON_WM_LBUTTONUP()
...
HTREEITEM CTreeCtrlDrag::CreateNewFolder()
{
	...
	if (m_bNoDuplicatesOnSameLevel)
	{
		...
		while (!IsUniqueNameOnThisLevel(strNewFolder, NULL, hParent))
			strNewFolder.Format(_T("%s %d"), FOLDER_BASE_NAME, nIndex++);
	}
...
void CTreeCtrlDrag::SuccessfulDrag()
{
	...
	else
	{
		...
		while (hParent != NULL)
		{
			...
			{
				AfxMessageBox(_T("Invalid drag & drop - the destination location can't be within the dragged item's sub-folder!"), MB_ICONEXCLAMATION);
				bGoingOK = FALSE;
				break;
			}
...
void CTreeCtrlDrag::CopySubtree(HTREEITEM hDest, HTREEITEM hSrc)
{
	...
	while (hChildSrc)
	{
		HTREEITEM hChildDest = InsertItem(_T("dest child"), hDest);
